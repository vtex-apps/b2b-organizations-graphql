 ...
 *
 * 1. Per-pod in-memory LRU, so a warm pod does no I/O at all.
 * 2. Optional cross-pod VBase stale-while-revalidate.
 *
 ...
 *
 * Failures and intentional misses must be thrown from `fetcher` so they are
 * never stored. Callers that mutate returned objects must clone first.
 */
